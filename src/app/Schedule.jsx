...
const Schedule = () => {
  ...
  const fetchAvailableMusics = async () => {
    try {
      const res = await axios.get("http://192.168.99.42:5000/songs-list");
      ...
      const promises = songs.map((song) => {
        return new Promise((resolve) => {
          const audio = new Audio(`http://192.168.99.42:5000${song.songSrc}`);
  ...
  const handleDeleteSong = async (song) => {
    ...
    try {
      ...
      await axios.delete(`http://192.168.99.42:5000/songs/${filename}`);
  ...
  const handleScheduleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://192.168.99.42:5000/schedules", schedulePayload, {
        headers: {
