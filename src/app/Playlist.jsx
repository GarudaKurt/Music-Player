...
const Playlist = () => {
  ...
  useEffect(() => {
    axios.get('http://10.168.99.42:5000/songs-list')
      .then(res => {
        ...
        if (res.data.length > 0 && !isScheduledPlaying.current) {
          ...
          if (currentAudio.current) {
            ...
            currentAudio.current.src = `http://10.168.99.42:5000${firstSong.songSrc}`;
            currentAudio.current.load();
          }
  ...
  const playScheduledSong = (song, scheduleName = '') => {
    ...
    audioEl.src = `http://10.168.99.42:5000${song.songSrc}`;
    audioEl.load();
  ...
  useEffect(() => {
    const checkAndPlayScheduledSong = async () => {
      ...
      try {
        const schedulesRes = await axios.get('http://10.168.99.42:5000/schedules');
  ...
  const handleOverrideClick = () => {
    if (isOverrideMode) {
      if (pausedScheduledDetails) {
        setCurrentMusicDetails(pausedScheduledDetails);
        currentAudio.current.src = `http://10.168.99.42:5000${pausedScheduledDetails.songSrc}`;
        currentAudio.current.load();
  ...
  const handleSelectOverrideSong = (song) => {
    ...
    currentAudio.current.src = `http://10.168.99.42:5000${song.songSrc}`;
    currentAudio.current.load();
    currentAudio.current.play();
    setIsAudioPlaying(true);
    lastPlayedTimestampRef.current = Date.now();
  ...
  const updateCurrentMusicDetails = (index, playNow = true) => {
    ...
    if (currentAudio.current) {
      currentAudio.current.src = `http://10.168.99.42:5000${music.songSrc}`;
      currentAudio.current.load();
      if (playNow) {
        currentAudio.current.play()
          .then(() => {
  ...
  const handleAudioPlay = async () => {
    if (currentAudio.current.paused) {
      try {
        await axios.post('http://10.168.99.42:5000/manual-play', { action: 'play' });
      ...
      try {
        await axios.post('http://10.168.99.42:5000/manual-play', { action: 'pause' });
